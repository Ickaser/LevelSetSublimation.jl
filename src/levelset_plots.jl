...
function heat(field, dom)
    return heatmap(dom.rgrid, dom.zgrid, field'; 
                    xlim=(dom.rmin,dom.rmax), ylim=(dom.zmin, dom.zmax),
                     aspect_ratio=:equal)
end
function plot_contour(phi, dom; lab="", c=:black)
    cl = levels(contours(dom.rgrid,dom.zgrid,phi, [0]))[1]
    for seg in lines(cl)
        xs, ys = coordinates(seg) # coordinates of this line segment
        plot!(xs, ys, color=c, label=lab) 
    end
end
freshplot(dom) = plot(aspect_ratio=:equal, 
                    xlim=(dom.rmin,dom.rmax), ylim=(dom.zmin,dom.zmax))

function plot_cylheat(T, dom)
...
function plot_cylcont(phi, dom; lab="", c=:black)
    cl = levels(contours(dom.rgrid,dom.zgrid,phi, [0]))[1]
    for seg in lines(cl)
        xs, ys = coordinates(seg) # coordinates of this line segment
        plot!(xs, ys, color=c, label=lab) 
...
function markfront(phi, dom; lab="", c=:white)
